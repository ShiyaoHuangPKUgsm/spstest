...
```python
spstest(D, X, model="probit", basis="T", data_driven=True, s=8)
```
...
In practice, the data-driven test is more recommended, as it automatically determines the testing order. 
`s` is a positive integer. If `data_driven = True`, `s` is the upper bound for selecting the optimal testing order (in this case, `s` is suggested to be less than or equal to 10). Otherwise, `s` is the specified testing order (we recommend $1 \leq s \leq 4$ in this case).
